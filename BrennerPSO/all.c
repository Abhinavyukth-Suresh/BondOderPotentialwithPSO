...
void optimize(Swarm* swarm, int n_iter,float c1,float c2,float w,float chi){ 
    ...
    float gbests[n_threads];
    float gbest_vec[n_threads][(*swarm).input_dim];
    int thread_num;
    for(int iter=0;iter<n_iter;iter++){
        ...
        int best_index = 0;
        for(int i=0;i<n_threads;i++){
            if(gbests[i]<gbests[best_index]){
                best_index = i;
            }
        }
        
        (*swarm).global_best = gbests[best_index];     
        for(int i=0;i<(*swarm).input_dim;i++){
            (*swarm).Gbest_vector[k] = gbest_vec[best_index][i];
        }
    }
}
