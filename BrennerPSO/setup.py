...
def setup(setup_=False,n=200):
    ...
    try:
        if os.path.isfile('CDLL/shared.so'):
            os.remove('CDLL/shared.so')
    except OSError as e:
    ...
    for i in list_C:
        print(f'compilie -fopenmp -c O3  -msse -mavx -mavx2  -o CDLL/{i[:-2]}.o {i}' )
    ...
    #status = os.system(f'gcc -shared -fPIC -O3  -msse -mavx -mavx2 -o CDLL/shared.so {cmp_str}')
    status = os.system(f'gcc -shared -fPIC -O3  -msse -mavx -mavx2  -D N={n} -D NTHREADS={ncpu} -o CDLL/shared.so all.c')
    if status != 0:
