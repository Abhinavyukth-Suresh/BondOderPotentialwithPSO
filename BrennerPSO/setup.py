...
def setup(setup_=False):
    ...
    for i in list_C:
        print(f'compilie -c O3  -msse -mavx -mavx2 -o CDLL/{i[:-2]}.o {i} -fopenmp')
        os.system(f'gcc -c -O3  -msse -mavx -mavx2  -o CDLL/{i[:-2]}.o {i} -fopenmp')
    ...
    print('compile -shared -fPIC -O3  -msse -mavx -mavx2 -o CDLL/shared.so '+cmp_str+' -fopenmp')
    status = os.system('gcc -shared -fPIC -O3  -msse -mavx -mavx2 -o CDLL/shared.so '+cmp_str+ ' -fopenmp')
    if status != 0:
        print("SETUP FAILED")
